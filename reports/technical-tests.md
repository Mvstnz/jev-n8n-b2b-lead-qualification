...
| TECH-03 Lost response after save | Not run | n8n Sheets append succeeded, but no lost-response recovery was exercised |
...
The first Gateway smoke returned 403. A subsequent smoke, one n8n end-to-end execution and 28 evaluation replies succeeded. Provider rate limits interrupted the rest of the evaluation; see [evaluation](evaluation.md). Technical statuses above distinguish a successful path from tested recovery behavior.
